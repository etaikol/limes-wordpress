    ...
    function convertCheckboxesToDropdown() {
        ...
        $('.wc-pao-addon.display-select.type-checkbox').each(function() {
            ...
            $addonGroup.find('p.wc-pao-addon-wrap').each(function() {
                ...
                // Move the original checkbox and label content to the dropdown
                var $newLabel = $('<label></label>');
                $newLabel.append($checkbox.detach()); // Move the original checkbox
                $newLabel.append(' <span>' + labelText + '</span>');
                
                $option.append($newLabel);
                $optionsContainer.append($option);
                
                // Remove the original wrapper completely
                $wrap.remove();
            });
            ...
            $optionsContainer.on('change', 'input[type="checkbox"]:not(.select-all-checkbox)', function() {
                // Just update the display text since we're using the original checkboxes
                updateSelectedText();
            });
            ...
            // Ensure all hidden checkboxes start enabled (not disabled)
